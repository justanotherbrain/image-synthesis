...
function ImageSynthesis(net, guide, init, iter_n, octave_n, octave_scale, end_layer, clip, visualize)
    ...
    if init == nil then init = torch.rand(3,h,w) end
...
function demo()
    --img = torch.Tensor(3,800,1240):zero()
    --image.display(image.toDisplayTensor{img})
    guide = image.load('guide.png')
    x = ImageSynthesis(net, guide, nil, 10, 4, 1.4, 34)
    --image.display(image.toDisplayTensor(x))
    for i=1,1000 do
        x = ImageSynthesis(net,guide, x, 15,1,1,1)
    end
